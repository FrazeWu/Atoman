...
导航栏使用**文字按钮**（不使用图标），显示「通知」，未读数 > 0 时在旁边显示红点角标数值。

```
[通知 3]   ← 导航栏右侧，3 = 未读数
```

点击后跳转至独立通知页 `/notifications`，不使用下拉面板。

### 通知页 `/notifications`

- Tab 筛选：全部 / 未读 / 回复 / @提及
- 每条通知：actor 用户名 + 动作描述 + 内容摘要 + 时间
- 点击单条：跳转到来源内容（帖子/回复），标记为已读
- 一键全部已读按钮
- 分页加载（首期不做无限滚动）
...
| Type | 触发时机 | 接收者 | 优先级 |
|------|---------|--------|--------|
| `forum_reply` | 有人回复了我的帖子或我的回复 | 帖主 / 被回复者 | 2 |
| `forum_mention` | 回复正文中 `@username` 提及了我 | 被提及者 | 3（最高）|
| `forum_solved` | 我的回复被标为解决方案 | 回复作者 | 1 |
...
| 方法 | 路径 | 描述 |
|------|------|------|
| GET | `/api/notifications` | 获取通知列表，`?page=1&type=all\|unread\|reply\|mention` |
| GET | `/api/notifications/unread-count` | 获取未读数（WS 断线回退 / 初始加载）|
...
职责：
- 应用启动时调用 `fetchUnreadCount()` 初始化铃铛角标
- 建立 `/ws/user` WS 连接（与私信共用）
- WS 收到 `notification` 事件：`unreadCount++`，若当前在 `/notifications` 页则 prepend 列表
- 提供 `markRead(id)`、`markAllRead()` action
...
- [x] `NotificationService`（创建、去重、@解析）
- [x] Forum 回复/解决方案事件触发通知
- [x] `/ws/user` WebSocket 端点（user hub）
- [x] REST API（列表、未读数、已读）
- [x] 前端 notificationStore
- [x] 导航栏「通知 N」文字按钮
- [x] `/notifications` 页面（分类 Tab + 列表 + 一键已读）
